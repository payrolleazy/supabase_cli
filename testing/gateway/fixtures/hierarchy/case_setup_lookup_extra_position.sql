...
begin
  ...
  execute format('select position_id from %I.hierarchy_position where position_code = $1 limit 1', v_schema_name)
  into v_position_id using 'hier-extra-pos-{{RUN_ID}}';
  if v_position_id is null then raise exception 'HIERARCHY extra position not found for run {{RUN_ID}}'; end if;
  insert into pg_temp.hierarchy_case_tokens (token_key, token_value) values ('HIER_EXTRA_POSITION_ID', v_position_id::text);
end;
$$;
