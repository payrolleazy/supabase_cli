...
declare
  ...
  v_is_shared_identity boolean := false;
begin
  if v_scenario = '' or v_fixture_run_key = '' then
  ...
  v_is_shared_identity := coalesce((v_identity->>'shared_identity')::boolean, false);
  ...
  where coalesce(metadata->>'fixture_run_key', '') = v_fixture_run_key
     or (v_is_shared_identity and v_user_id is not null and actor_user_id = v_user_id)
     or (not v_is_shared_identity and lower(identifier) = lower(v_identity->>'email'))
     or (not v_is_shared_identity and v_user_id is not null and actor_user_id = v_user_id);

  delete from public.platform_identity_event_log
  where coalesce(details->>'fixture_run_key', '') = v_fixture_run_key
     or (v_is_shared_identity and v_user_id is not null and actor_user_id = v_user_id)
     or (not v_is_shared_identity and v_user_id is not null and actor_user_id = v_user_id);

  if not v_is_shared_identity then
  ...
  return jsonb_build_object(
    ...
    'data', jsonb_build_object(
      ...
      'shared_identity', v_is_shared_identity
    )
  );
end;
$$;
...
begin
  ...
  if v_user_id is null then
    return jsonb_build_object('success', false, 'error', 'SHARED_SIGNIN_IDENTITY_NOT_FOUND', 'details', jsonb_build_object('email', v_identity->>'email'));
  end if;
