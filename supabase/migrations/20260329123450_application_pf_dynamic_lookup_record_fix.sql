...
begin
  ...
  if v_batch.batch_id is null then return public.platform_json_response(false,'BATCH_NOT_FOUND','PF batch was not found.',jsonb_build_object('batch_id', v_batch_id)); end if;
...
begin
  ...
  if v_run.ecr_run_id is null then return public.platform_json_response(false,'ECR_RUN_NOT_FOUND','PF ECR run was not found.',jsonb_build_object('ecr_run_id', v_run_id)); end if;
