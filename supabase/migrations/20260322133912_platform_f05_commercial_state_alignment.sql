...
begin
  ...
  if v_access_row.billing_state is distinct from v_new_billing_state then
    perform public.platform_append_status_history(v_tenant_id, 'billing', v_access_row.billing_state, v_new_billing_state, 'COMMERCIAL_DUES_OVERDUE', jsonb_build_object('overdue_since', v_overdue_since, 'background_stop_from', v_background_stop_from), v_actor, 'platform_sync_tenant_access_from_commercial');
  end if;
